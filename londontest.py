...
def sql_insert(con, table_name, ath):
    ...
    (country, club, category, id, place_gender, place_category, place_overall, gender,
...
def get_ath_data(ath_soup):
    ...
    vars = ['Name', 'Club', 'Category', 'Runner Number', 'Place (Gender)', 'Place (Category)', 'Place (Overall)']
    ...
    for s in soup:
        if s.text == 'Name':
            if name_found:
                ...
                try:
                    club = re.search(r'\(([A-Z]+)\)', name).group(1)
                    name_found = True
                except:
                    flag = True
                    club = 'None'
    ...
    vars = ['5K', '10K', '15K', '20K', 'Half', '25K', '30K', '35K', '40K', 'Finish']
    soup = ath_soup.find_all('th', text=vars)
    for s in soup:
